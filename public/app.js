...
function getAuthToken() {
  return localStorage.getItem('wbAuthToken') || null;
}
...
async function checkAuth() {
  ...
  if (urlToken) {
    localStorage.setItem('wbAuthToken', urlToken);
    window.history.replaceState({}, '', window.location.pathname);
  }

  try {
    const res = await fetch('/auth/status', { headers: getAuthHeaders() });
    ...
    // If google not configured, skip login screen entirely
  } catch (e) {}
}
...
function showLoginScreen() {
  ...
  screen.innerHTML = `
    <div class="login-box">
      ...
      <h1>Workout Buddy</h1>
      <p>Your AI-powered Ironman training dashboard</p>
      ...
      <p class="login-hint">Sign in to sync your data across devices. Or:</p>
      <button class="btn btn-secondary" onclick="document.getElementById('login-screen').remove()" style="margin-top:8px;width:100%">Continue without signing in</button>
    </div>
  `;
...
function logout() {
  localStorage.removeItem('wbAuthToken');
...
function copySyncLink() {
  ...
  navigator.clipboard.writeText(link).then(() => {
    ...
    showToast('✅ Link copied — open it on your other device to sync.');
  }).catch(() => {
...
async function clearHistory() {
  ...
  await fetch('/api/history/clear', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ userId: getUserId() }) });
  const chatWindow = document.getElementById('chat-window');
...
async function stravaSync() {
  ...
  try {
    const res  = await fetch('/api/strava/sync', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ userId: getUserId() }) });
    const data = await res.json();
...
async function stravaDisconnect() {
  ...
  await fetch('/auth/strava/disconnect', { method: 'POST', headers: getAuthHeaders(), body: JSON.stringify({ userId: getUserId() }) });
  checkStravaStatus();
}
